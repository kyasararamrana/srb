...
class Bagtype_Model extends CI_Model
{
  ...
  public  function update($id,$data){
	 $this->db->where('id',$id); 
	 return $this->db->update('ecom_bagtype',$data);
  }
  ...
  {
    $this->db->select('id,other_charges,printing_cost,block_charges,additional_gsm,percentage,cost_per_kg');
    return $this->db->get_where($this->table, array('id' => $bag_type))->row();
  } 
  //get all details by bag type
  public function get_b_additional_details($bag_type='')
  {
    $this->db->select('*');
    return $this->db->get_where($this->table, array('id' => $bag_type))->row_array();
  }
  // material list
